...
export const register = async (req: Request, res: Response) => {
  ...
  res.json({ success: true, data: { _id: user._id, email: user.email, token: generateToken(user._id.toString()) }, message: "Registered" });
};
...
export const login = async (req: Request, res: Response) => {
  ...
  if (await bcrypt.compare(password, user.password)) {
    res.json({ success: true, data: { _id: user._id, email: user.email, token: generateToken(user._id.toString()) }, message: "Logged in" });
  } else {
